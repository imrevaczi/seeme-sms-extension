...
// Load the official seeme library
require_once 'Services/seeme/autoload.php';
use seeme\Rest\Client;
 ...
 *
 */
class org_civicrm_sms_seeme extends CRM_SMS_Provider {
  ...
  public $_apiURL = "https://api.seeme.com/";
   ...
   * Constructor
   *
   * Create and auth a seeme session.
   * This is not needed for seeme
   *
   * @return void
   */ 
  function __construct($provider = array(
    ...
    if ($this->_apiType == 'http' &&
      ...
      $this->_seemeClient = new Client($sid, $token);
    }
  ...
  static function &singleton($providerParams = array(
    ...
    if (!isset(self::$_singleton[$cacheKey]) || $force) {
      ...
      self::$_singleton[$cacheKey] = new org_civicrm_sms_seeme($provider, $skipAuth);
    }
    return self::$_singleton[$cacheKey];
  }

  /**
   * Authenticate to the seeme Server.
   * Not needed in seeme
   * @return boolean TRUE
   * @access public
   * @since 1.1
  ...
  /**
   * Send an SMS Message via the seeme API Server
   *
  ...
  function send($recipients, $header, $message, $jobID = NULL, $userID = NULL) {
    if ($this->_apiType == 'http') {
      ...
      try {
        $seemeMessage = $this->_seemeClient->messages->create(
          $header['To'],
          array(
            'from' => $from,
            'body' => $message
